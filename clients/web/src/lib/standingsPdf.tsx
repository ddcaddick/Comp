...
const styles = StyleSheet.create({
  ...
  dropNote: { fontSize: 7, color: colors.accent, marginBottom: 4 },
});
  ...
  return (
    <View style={pdfStyles.tile}>
      ...
      <View style={pdfStyles.headerRow}>
        <Text style={[pdfStyles.headerCell, { width: "15%" }]}>Pos</Text>
        <Text style={[pdfStyles.headerCell, { width: "45%" }]}>Shooter</Text>
        <Text style={[pdfStyles.headerCell, { width: "20%", textAlign: "right" }]}>Dropped</Text>
        <Text style={[pdfStyles.headerCell, { width: "20%", textAlign: "right" }]}>Points</Text>
      </View>
      {sorted.map((s) => (
        <View key={s.shooterId} style={pdfStyles.row}>
          <Text style={[pdfStyles.cell, { width: "15%" }]}>{s.position}</Text>
          <Text style={[pdfStyles.cell, { width: "45%" }]}>
            {preferredName(s)}
            {s.isProvisional ? " (prov.)" : ""}
          </Text>
          <Text style={[pdfStyles.cellMuted, { width: "20%", textAlign: "right" }]}>{s.missedEvents}</Text>
          <Text style={[pdfStyles.cell, { width: "20%", textAlign: "right" }]}>{s.countingTotal}</Text>
        </View>
      ))}
